...
class IPPOAgent:
    ...
    def compute_gae(self):
        T = len(self.rewards)
        num_envs = self.rewards[0].shape[0]  # 获取环境数量
    ...
    def update(self):
        ...
        batch_size = len(self.observations) * self.rewards[0].shape[0]  # T * num_envs
        ...
        while not done:
            ...
            for i, agent in enumerate(self.agents):
                ...
                agent.store_transition(
                    obs[i],
                    action,
                    rewards[i] if 'rewards' in locals() else torch.zeros(self.env.num_envs, device=self.device),  # 初始奖励
                    value,
                    log_prob,
                    torch.zeros(self.env.num_envs, device=self.device)  # 初始done状态，直接使用浮点类型
                )
            ...
            for i in range(self.n_agents):
                agent.rewards[-1] = rewards[i].reshape(-1, 1)  # 确保reward是[B, 1]形状
                agent.dones[-1] = torch.tensor(done, device=self.device, dtype=torch.float).expand(self.env.num_envs).reshape(-1, 1)  # 使用浮点类型
...
def plot_rewards(rewards, window_size=100):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('ippo_training_rewards.png')
    plt.close()
...
def main():
    ...
    pygame.display.set_caption("VMAS Balance IPPO Training")
    ...
    # 创建日志文件
    log_file = open('ippo_training_log.txt', 'w')
    ...
    for episode in range(num_episodes):
        ...
        if avg_reward > best_avg_reward:
            ...
            torch.save(checkpoint, 'ippo_best_model.pt')
    ...
    torch.save(final_checkpoint, 'ippo_final_model.pt')
